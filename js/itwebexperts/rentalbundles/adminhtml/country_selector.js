var RentalBundlesCountrySelector = Class.create();
RentalBundlesCountrySelector.prototype = {
    config: {
...
var RentalBundlesSimSelection = Class.create();
RentalBundlesSimSelection.prototype = {
    config: {},
    ...
    unSelectAll: function () {
        this.simSelector.setValue([]);
    },

    selectMultiValue: function (sims) {
        if (sims instanceof Array) {
            this.simSelector.setValue(sims.concat(this.simSelector.getValue()));
        }
    },

    onCountryChange: function (select, event) {
        if (typeof this.countrySims == 'undefined') return;
        var _this = this;
        this.unSelectAll();
        this.countrySelectors.each(function (el) {
            if (el.value && _this.countrySims[el.value]) {
                var sims = _this.countrySims[el.value];
                _this.selectMultiValue(sims);
            }
        }.bind(this));
    },
    ...
    init: function () {
        this.countrySelectors = $$(this.config.countrySelectorClass);
        this.simSelector = $$(this.config.simSelectorClass).first();
